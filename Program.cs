...
try
{
    ...
    // Add registrations for Lazy<T> to resolve circular dependencies
    builder.Services.AddScoped<Lazy<VehicleMapper>>(provider => 
        new Lazy<VehicleMapper>(() => provider.GetRequiredService<VehicleMapper>()));
